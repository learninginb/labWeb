...
<script>
	...
	//添加
	function toAdd(){
		window.location='${path}/knowledgeController/toAdd.do';
	}
	//删除
	function toRemove(){
		var ids=getSelectedRowsIds('KnowledgeList');
		if(ids){
			top.showConfirmDiaglog('提示','您将删除被选择的纪录及其历史版本，删除数据将进入回收站，确定要删除吗？',function(){  //关闭事件
				refleshData('KnowledgeList');
			},function(){  //确认事件
				 $.post('${path}/knowledgeController/updateState.do?ids='+ids+'&knowledgeState=7',function(data){
	...
	//编辑
    function toEdit(){
    	var selects = $("#KnowledgeList").bootstrapTable('getSelections');//获取表选择的行
    	var selected=getSelectedRowsArr('KnowledgeList');
    	if(selected.length>0&&selected.length<2){
    		if(selects[0].knowledgeState==0 || selects[0].knowledgeState==5){//只能修改待审核和被驳回的信息点
        		window.location='${path}/knowledgeController/editById.do?id='+selected;
    		}else{
        		showArtDiaglog('提示','对不起，此状态下的信息不能被修改',null,function(){closeDialog();});
    		}
    	}else{
    		showArtDiaglog('提示','请选择一条数据进行操作',null,function(){closeDialog();});
    	}
	}
	...
	function deleteById(id){
		showConfirmDiaglog('提示','您将删除这条记录及历史版本，删除数据将进入回收站，确定要删除吗？',function(){  //关闭事件
				refleshData('KnowledgeList');
			},function(){  //确认事件
				 $.post('${path}/knowledgeController/updateState.do?ids='+id+'&knowledgeState=7',function(data){
				   var json=$.parseJSON(data);
				   if(json.success){
					   showArtDiaglog('提示','删除成功',function(){  //关闭事件
	 					},function(){   //确定事件
    ...
    function operatorFormatter(value, row, index) {
    	var operator='';
		<shiro:hasPermission name="MyKnowledge:info">
		...
		<shiro:hasPermission name="MyKnowledge:remove">
		...
		<shiro:hasPermission name="MyKnowledge:viewHistory">
			operator += '<button class="btn btn-info btn-round btn-xs" onclick="toHistoryById(\''+row.id+'\')"><i class="fa fa-search"></i>查看历史版本</button>&nbsp;&nbsp;'
		</shiro:hasPermission>
		<shiro:hasPermission name="MyKnowledge:edit">
			if(row.knowledgeState==0 || row.knowledgeState==5){
				...
				operator += '<button class="btn btn-warning btn-round btn-xs" onclick="editById(\''+row.id+'\');" disabled="disabled"><i class="glyphicon glyphicon-pencil"></i> 修改</button>&nbsp;&nbsp;';
			}
		</shiro:hasPermission>
		return operator
    }
	...
	function changeList(pid,parentName,dictType){
		if(!selectNode){
			...
			$('#KnowledgeList').bootstrapTable('refresh',{url:"${path}/knowledgeController/list.do?knowledgeType="+pid+"&createUser="+createUser});
		} 
	}
    ...
    <div id="resource-left">
		...
		<div id="resource-right">
			<div>
	    		...
	    		<input type="hidden" name="createUser" id="createUser" value="${currentUser }" />
	    	</div>
		    <div id="toolbar" class="btn-group">
		    <shiro:hasPermission name="MyKnowledge:add">
	    	...
	    	<table id="KnowledgeList" data-toggle="table"
				data-url="${path}/knowledgeController/list.do?createUser=${currentUser}" data-pagination="true"
