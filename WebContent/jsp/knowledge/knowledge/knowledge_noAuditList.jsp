...
<script>
	 ...
	 function toHistory(){
		...
		function auditById(id) {
			var dialog = art.dialog({
				...
				cancelVal : "不通过",
				ok : function() {
					$.post('${path}/knowledgeController/updateState.do?ids='+id+'&knowledgeState=1',function(data){
						    ...
						    if(json.success){
							   ...
							   showArtDiaglog('提示','审核通过失败',function(){
			 					},function(){
			 						closeDialog();
			 					});
						    }
				...
				cancel : function() {
					$.post('${path}/knowledgeController/updateState.do?ids='+id+'&knowledgeState=5',function(data){
						var json=$.parseJSON(data);
					    if(json.success){
						   ...
						   showArtDiaglog('提示','审核不通过失败',function(){
		 					},function(){
		 						closeDialog();
		 					});
					    }
		...
		function toAudit() {
			...
			if (ids) {
				...
				showArtDiaglog('提示', '请至少选择一条数据进行操作', null, function() {
					closeDialog();
				});
			}
		}
    ...
    function operatorFormatter(value, row, index) {
    	var operator='';
		<shiro:hasPermission name="NoauditKnowledge:info">
