...
<script>
    ...
    function toInfo(){
    	...
    	if(selected.length>0&&selected.length<2){
    		window.location='${path}/knowledgeController/readById.do?id='+selected;
    	}else{
	...
	function viewById(id){
			window.location='${path}/knowledgeController/readById.do?id='+id;
	}
	//根据id历史版本
    ...
    function operatorFormatter(value, row, index) {
    	var operator='';
		<shiro:hasPermission name="PublishedKnowledge:read">
			operator += '<button class="btn btn-success btn-round btn-xs" onclick="viewById(\''+row.id+'\')"><i class="glyphicon glyphicon-list-alt"></i>阅读</button>&nbsp;&nbsp;'
		</shiro:hasPermission>
		return operator;
    }
