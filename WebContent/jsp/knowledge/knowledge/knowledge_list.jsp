...
<script>
	...
	function deleteById(id){
		top.showConfirmDiaglog('提示','您将删除这条记录及历史版本，删除数据将进入回收站，确定要删除吗？',function(){  //关闭事件
				refleshData('KnowledgeList');
			},function(){  //确认事件
	 ...
	 function toHistory(){
	  ...
	  //操作工具栏
	    function operatorFormatter(value, row, index) {
			...
			<shiro:hasPermission name="Knowledge:info">
			...
			<shiro:hasPermission name="Knowledge:remove">
			...
			<shiro:hasPermission name="Knowledge:viewHistory">
				operator += '<button class="btn btn-info btn-round btn-xs" onclick="toHistoryById(\''+row.id+'\')"><i class="fa fa-search"></i>查看历史版本</button>&nbsp;&nbsp;'
			</shiro:hasPermission>
			<shiro:hasPermission name="Knowledge:audit">
				if(row.knowledgeState==0){
					operator += '<button class="btn grey-cascade btn-xs" onclick="auditById(\''+row.id+'\')"><i class="fa fa-link"></i>审核</button>'
				}
			</shiro:hasPermission>
			return operator
	    }
	...
	function changeList(pid,parentName,dictType){
		if(!selectNode){
			...
			$('#KnowledgeList').bootstrapTable('refresh',{url:"${path}/knowledgeController/list.do?knowledgeType="+pid});
		} 
	}
...
<body>
    ...
    <div id="resource-left">
		...
		<div id="resource-right">
			...
			<%-- <shiro:hasPermission name="Knowledge:audit">
			    <button class="btn grey-cascade" onclick="toAudit()">
					<i class="fa fa-link"></i>审核
			    </button>
			</shiro:hasPermission> --%>
			</div>
	    
	    	<table id="KnowledgeList" data-toggle="table"
				data-url="${path}/knowledgeController/list.do" data-pagination="true"
