...
function requestData(){
	getAjax("${path}/TaskController/requestAddSonData.do",{parentId:${param.id}},(data)=>{
		parentTask = data['taskVo'];
		if(parentTask!=null){
		    ...
		    $('.selectpicker').selectpicker('render');
		  	$('#parentTask').val(parentTask.theme);
		  	$('#type').val(parentTask.type);
		  	var importaceSelect = document.getElementById("importance");
		  	$('#importance').html('');
		  	for(var i=1;i<=parentTask.importance;i++){
		  		importaceSelect.add(new Option(i,i))
		  	}
		    $('.form_date').datetimepicker("setEndDate",new Date(parentTask.finishTime));
		    $('#pic').val(parentTask.pic);
		    }else{
	    	showArtDiaglog('提示', '父任务不存在', null, function() {
			closeDialog();
			});}
	});
}
...
function toSumbit(){
	if(formValidate()){
		var fromData = $("#add_form").serialize();
		var Data = $.param({'parentId':parentTask.id,'picId':parentTask.picId})+'&'+fromData;
		console.log(Data);
		$("#add_form").ajaxSubmit({
			url : "${path}/TaskController/addSonTask.do?parentId="+parentTask.id+"&picId="+parentTask.picId,
			data :fromData,
			cache : false,
			dataType : 'json',
			type : 'post',
	...
	<div class="formbody">
			...
			<form id="add_form" method="post" enctype ="multipart/form-data">
				<ul class="forminfo" >
					...
					<li><label>父任务：</label>
						<input id="parentTask" value="父任务主题" type="text" class="form-control input-primary" style="width: 350px;"  readonly="readonly">
					</li>
					<li><label>重要程度：</label>
					...
					<li><label>类型：</label>
						<input id="type" name="type" value="计划性任务" type="text" class="form-control input-primary" style="width: 350px;"  readonly="readonly">
					</li>
					<li><label>负责人：</label>
						<input id="pic" value="负责人" type="text" class="form-control input-primary" style="width: 350px;"  readonly="readonly">
					</li>
					<li><label>分配人：</label>
